import React from "react";
import { motion, AnimatePresence } from "motion/react";
import { X, Plus, History, Users, Trash2, Settings, BookMarked, UserPlus, Calendar, UserCircle, Heart, Palette, Brain } from "lucide-react";
import { ChatSession, SideCharacter, CustomSideCharacter } from "../types";
import { SIDE_CHARACTERS } from "../constants";
import { cn } from "../lib/utils";

interface SidebarProps {
  isOpen: boolean;
  onClose: () => void;
  sessions: ChatSession[];
  currentSessionId: string;
  onSelectSession: (id: string) => void;
  onDeleteSession: (id: string) => void;
  onNewChat: () => void;
  onChangeApiKey: () => void;
  notebookEvents: string[];
  onAddNotebookEvent: (event: string) => void;
  onDeleteNotebookEvent: (index: number) => void;
  customCharacters: CustomSideCharacter[];
  onAddCustomCharacter: (char: CustomSideCharacter) => void;
  onDeleteCustomCharacter: (index: number) => void;
}

export const Sidebar: React.FC<SidebarProps> = ({
  isOpen,
  onClose,
  sessions,
  currentSessionId,
  onSelectSession,
  onDeleteSession,
  onNewChat,
  onChangeApiKey,
  notebookEvents,
  onAddNotebookEvent,
  onDeleteNotebookEvent,
  customCharacters,
  onAddCustomCharacter,
  onDeleteCustomCharacter,
}) => {
  const [activeTab, setActiveTab] = React.useState<"history" | "characters" | "notebook">("history");
  const [isAddingChar, setIsAddingChar] = React.useState(false);
  const [newChar, setNewChar] = React.useState<CustomSideCharacter>({
    name: "",
    gender: "Nam",
    role: "",
    description: "",
    birthDate: "",
    appearance: "",
    personality: "",
  });
  const [newEvent, setNewEvent] = React.useState("");
  const [isAddingEvent, setIsAddingEvent] = React.useState(false);

  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 bg-black/20 backdrop-blur-sm z-40"
          />
          <motion.div
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
            transition={{ type: "spring", damping: 25, stiffness: 200 }}
            className="fixed right-0 top-0 h-full w-80 bg-[#FFF0F5] shadow-2xl z-50 flex flex-col border-l border-pink-100"
          >
            <div className="p-4 flex items-center justify-between border-bottom border-pink-100">
              <h2 className="text-lg font-bold text-[#333]">Menu</h2>
              <button onClick={onClose} className="p-2 hover:bg-pink-100 rounded-full transition-colors">
                <X className="w-5 h-5 text-[#333]" />
              </button>
            </div>

            <div className="flex border-b border-pink-100">
              <button
                onClick={() => setActiveTab("history")}
                className={cn(
                  "flex-1 py-3 flex items-center justify-center gap-2 text-sm font-medium transition-colors",
                  activeTab === "history" ? "text-[#ff99cc] border-b-2 border-[#ff99cc]" : "text-gray-500"
                )}
              >
                <History className="w-4 h-4" />
                Lịch sử
              </button>
              <button
                onClick={() => setActiveTab("characters")}
                className={cn(
                  "flex-1 py-3 flex items-center justify-center gap-2 text-sm font-medium transition-colors",
                  activeTab === "characters" ? "text-[#ff99cc] border-b-2 border-[#ff99cc]" : "text-gray-500"
                )}
              >
                <Users className="w-4 h-4" />
                Nhân vật
              </button>
              <button
                onClick={() => setActiveTab("notebook")}
                className={cn(
                  "flex-1 py-3 flex items-center justify-center gap-2 text-sm font-medium transition-colors",
                  activeTab === "notebook" ? "text-[#ff99cc] border-b-2 border-[#ff99cc]" : "text-gray-500"
                )}
              >
                <BookMarked className="w-4 h-4" />
                Sổ tay
              </button>
            </div>

            <div className="flex-1 overflow-y-auto p-4">
              {activeTab === "history" && (
                <div className="space-y-3">
                  <button
                    onClick={() => {
                      onNewChat();
                      onClose();
                    }}
                    className="w-full py-3 px-4 bg-white border border-pink-200 rounded-xl flex items-center gap-3 text-[#333] hover:bg-pink-50 transition-colors shadow-sm"
                  >
                    <Plus className="w-5 h-5 text-[#ff99cc]" />
                    <span className="font-medium">Chat mới</span>
                  </button>

                  <div className="pt-4 space-y-2">
                    {sessions.map((session) => (
                      <div
                        key={session.id}
                        className={cn(
                          "group relative p-3 rounded-xl border transition-all cursor-pointer",
                          session.id === currentSessionId
                            ? "bg-white border-[#ff99cc] shadow-md"
                            : "bg-white/50 border-pink-100 hover:border-pink-200"
                        )}
                        onClick={() => {
                          onSelectSession(session.id);
                          onClose();
                        }}
                      >
                        <div className="pr-8">
                          <p className="font-bold text-[#333] truncate">{session.name}</p>
                          <p className="text-xs text-gray-500 mt-1">
                            {new Date(session.lastUpdate).toLocaleString("vi-VN")}
                          </p>
                        </div>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            onDeleteSession(session.id);
                          }}
                          className="absolute right-2 top-1/2 -translate-y-1/2 p-2 opacity-0 group-hover:opacity-100 hover:text-red-500 transition-all"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
                  </div>
                </div>
              )}

              {activeTab === "characters" && (
                <div className="space-y-4">
                  <button
                    onClick={() => setIsAddingChar(!isAddingChar)}
                    className="w-full py-3 px-4 bg-white border border-pink-200 rounded-xl flex items-center gap-3 text-[#333] hover:bg-pink-50 transition-colors shadow-sm mb-4"
                  >
                    <UserPlus className="w-5 h-5 text-[#ff99cc]" />
                    <span className="font-medium">Thêm nhân vật phụ</span>
                  </button>

                  <AnimatePresence>
                    {isAddingChar && (
                      <motion.div
                        initial={{ height: 0, opacity: 0 }}
                        animate={{ height: "auto", opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
                        className="bg-white p-4 rounded-xl border border-pink-200 shadow-inner space-y-3 mb-6 overflow-hidden"
                      >
                        <div className="grid grid-cols-2 gap-2">
                          <input
                            placeholder="Tên"
                            value={newChar.name}
                            onChange={(e) => setNewChar({ ...newChar, name: e.target.value })}
                            className="text-xs p-2 bg-pink-50 rounded-lg border border-pink-100"
                          />
                          <select
                            value={newChar.gender}
                            onChange={(e) => setNewChar({ ...newChar, gender: e.target.value })}
                            className="text-xs p-2 bg-pink-50 rounded-lg border border-pink-100"
                          >
                            <option value="Nam">Nam</option>
                            <option value="Nữ">Nữ</option>
                            <option value="Khác">Khác</option>
                          </select>
                        </div>
                        <input
                          placeholder="Ngày sinh (VD: 12/10/1900)"
                          value={newChar.birthDate}
                          onChange={(e) => setNewChar({ ...newChar, birthDate: e.target.value })}
                          className="text-xs p-2 w-full bg-pink-50 rounded-lg border border-pink-100"
                        />
                        <input
                          placeholder="Mối quan hệ"
                          value={newChar.role}
                          onChange={(e) => setNewChar({ ...newChar, role: e.target.value })}
                          className="text-xs p-2 w-full bg-pink-50 rounded-lg border border-pink-100"
                        />
                        <textarea
                          placeholder="Ngoại hình"
                          value={newChar.appearance}
                          onChange={(e) => setNewChar({ ...newChar, appearance: e.target.value })}
                          className="text-xs p-2 w-full bg-pink-50 rounded-lg border border-pink-100 h-16 resize-none"
                        />
                        <textarea
                          placeholder="Tính cách"
                          value={newChar.personality}
                          onChange={(e) => setNewChar({ ...newChar, personality: e.target.value })}
                          className="text-xs p-2 w-full bg-pink-50 rounded-lg border border-pink-100 h-16 resize-none"
                        />
                        <button
                          onClick={() => {
                            if (newChar.name) {
                              onAddCustomCharacter({
                                ...newChar,
                                description: `Ngày sinh: ${newChar.birthDate}. Ngoại hình: ${newChar.appearance}. Tính cách: ${newChar.personality}`,
                              });
                              setNewChar({ name: "", gender: "Nam", role: "", description: "", birthDate: "", appearance: "", personality: "" });
                              setIsAddingChar(false);
                            }
                          }}
                          className="w-full py-2 bg-[#ff99cc] text-white rounded-lg text-xs font-bold"
                        >
                          Lưu nhân vật
                        </button>
                      </motion.div>
                    )}
                  </AnimatePresence>

                  <div className="space-y-4">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1">Nhân vật mặc định</p>
                    {SIDE_CHARACTERS.map((char, i) => (
                      <div key={i} className="bg-white p-4 rounded-xl border border-pink-100 shadow-sm">
                        <div className="flex justify-between items-start mb-1">
                          <p className="font-bold text-[#333]">{char.name}</p>
                          <span className="text-[10px] bg-pink-50 text-[#ff99cc] px-2 py-0.5 rounded-full font-bold">
                            {char.gender}
                          </span>
                        </div>
                        <p className="text-xs text-[#ff99cc] font-medium mb-2">{char.role}</p>
                        <p className="text-sm text-gray-600 leading-relaxed">{char.description}</p>
                      </div>
                    ))}

                    {customCharacters.length > 0 && (
                      <>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-1 pt-4">Nhân vật của bạn</p>
                        {customCharacters.map((char, i) => (
                          <div key={i} className="bg-white p-4 rounded-xl border border-[#ff99cc]/30 shadow-sm relative group">
                            <div className="flex justify-between items-start mb-1">
                              <p className="font-bold text-[#333]">{char.name}</p>
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] bg-pink-50 text-[#ff99cc] px-2 py-0.5 rounded-full font-bold">
                                  {char.gender}
                                </span>
                                <button
                                  onClick={() => onDeleteCustomCharacter(i)}
                                  className="text-gray-300 hover:text-red-500 transition-colors"
                                >
                                  <Trash2 className="w-3 h-3" />
                                </button>
                              </div>
                            </div>
                            <p className="text-xs text-[#ff99cc] font-medium mb-2">{char.role}</p>
                            <div className="space-y-2">
                              <div className="flex items-start gap-2 text-[11px] text-gray-500">
                                <Calendar className="w-3 h-3 mt-0.5 flex-shrink-0" />
                                <span>{char.birthDate}</span>
                              </div>
                              <div className="flex items-start gap-2 text-[11px] text-gray-500">
                                <Palette className="w-3 h-3 mt-0.5 flex-shrink-0" />
                                <span>{char.appearance}</span>
                              </div>
                              <div className="flex items-start gap-2 text-[11px] text-gray-500">
                                <Brain className="w-3 h-3 mt-0.5 flex-shrink-0" />
                                <span>{char.personality}</span>
                              </div>
                            </div>
                          </div>
                        ))}
                      </>
                    )}
                  </div>
                </div>
              )}

              {activeTab === "notebook" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between px-1">
                    <p className="text-sm font-bold text-[#333]">Sổ tay sự kiện ({notebookEvents.length}/20)</p>
                    <button
                      onClick={() => setIsAddingEvent(!isAddingEvent)}
                      disabled={notebookEvents.length >= 20}
                      className="p-1.5 bg-[#ff99cc] text-white rounded-full disabled:opacity-50"
                    >
                      <Plus className="w-4 h-4" />
                    </button>
                  </div>

                  <AnimatePresence>
                    {isAddingEvent && (
                      <motion.div
                        initial={{ height: 0, opacity: 0 }}
                        animate={{ height: "auto", opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
                        className="overflow-hidden"
                      >
                        <div className="bg-white p-3 rounded-xl border border-pink-200 shadow-sm space-y-2">
                          <textarea
                            value={newEvent}
                            onChange={(e) => setNewEvent(e.target.value)}
                            placeholder="Nhập sự kiện mới (VD: {{user}} đang bệnh...)"
                            className="w-full h-20 p-2 text-xs bg-pink-50 border border-pink-100 rounded-lg focus:outline-none resize-none"
                          />
                          <div className="flex gap-2">
                            <button
                              onClick={() => {
                                if (newEvent.trim()) {
                                  onAddNotebookEvent(newEvent.trim());
                                  setNewEvent("");
                                  setIsAddingEvent(false);
                                }
                              }}
                              className="flex-1 py-2 bg-[#ff99cc] text-white rounded-lg text-xs font-bold"
                            >
                              Thêm
                            </button>
                            <button
                              onClick={() => setIsAddingEvent(false)}
                              className="px-4 py-2 bg-gray-100 text-gray-500 rounded-lg text-xs font-bold"
                            >
                              Hủy
                            </button>
                          </div>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>

                  <div className="space-y-2">
                    {notebookEvents.map((event, i) => (
                      <div key={i} className="bg-white p-3 rounded-xl border border-pink-100 shadow-sm flex justify-between gap-3 group">
                        <p className="text-xs text-gray-600 leading-relaxed">{event}</p>
                        <button
                          onClick={() => onDeleteNotebookEvent(i)}
                          className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all flex-shrink-0"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
                    {notebookEvents.length === 0 && (
                      <div className="text-center py-12">
                        <BookMarked className="w-12 h-12 text-pink-100 mx-auto mb-3" />
                        <p className="text-xs text-gray-400">Chưa có sự kiện nào được ghi lại.</p>
                      </div>
                    )}
                  </div>
                </div>
              )}
            </div>

            <div className="p-4 border-t border-pink-100">
              <button
                onClick={() => {
                  onChangeApiKey();
                  onClose();
                }}
                className="w-full py-3 px-4 bg-pink-50 text-[#ff99cc] rounded-xl flex items-center justify-center gap-3 font-bold hover:bg-pink-100 transition-colors"
              >
                <Settings className="w-5 h-5" />
                Cài đặt API Key
              </button>
            </div>
          </motion.div>
        </>
      )}
    </AnimatePresence>
  );
};
